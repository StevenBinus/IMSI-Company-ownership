...
import {
	getAllStorageType,
	StorageTypeProps,
} from './api/getAllStorageType.api'
import { getStorageTypeById } from './api/getStorageTypeById.api'
...
const StorageTypeMasterPage = () => {
	...
	const handleOpenListModal = async () => {
		setIsLoading(true)
		const storageTypeData = await getAllStorageType()
	...
	const handleOnDoubleClickListModal = (row: StorageTypeProps) => {
		if (row !== null) {
	...
	const handleDataRowIdListModal = (row: StorageTypeProps) => {
		return row.storage_type_id
	}

	const handleOnBlurStorageType = async () => {
		setIsLoading(true)
		const storageTypeData = await getStorageTypeById(
	...
	return (
		<>
			...
			<Box sx={{ maxWidth: '540px', padding: '1%' }}>
				...
				/>
				<TextFieldDMS
					disabled={formik.values.storage_type_id !== null}
...
export default StorageTypeMasterPage
